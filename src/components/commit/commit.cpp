...
void Commit::onError(const QString &message, ErrorTag tag)
{
  ...
  QString extendedMessage = message.isEmpty()
    ? QObject::tr("There was no error output.")
    : QObject::tr("The following error output was generated by the commit call: \n") + message;

  QMessageBox dialog(
    QMessageBox::Warning,
    QObject::tr("Error on Commit for repository ") + static_cast<GitInterface*>(QObject::sender())->name(),
