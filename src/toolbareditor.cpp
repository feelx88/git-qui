...
_impl(new ToolBarEditorPrivate)
{
  ...
  auto addSlot = [=]{
    ...
    QAction *action = ToolBarActions::byId(item->data(0, Qt::UserRole).toString());
  ...
  for(auto &[id, action] : ToolBarActions::all().toStdMap()) {
    if (ui->treeWidget_2->findItems(action->text(), Qt::MatchExactly, 0).size() == 0)
    {
      _impl->createItem(ui->treeWidget, action);
    }
  }
